"""
This AST based model is an adaption to the original AST with the following adaption(s):
    The number of layers in the transformer encoder is adjustable
"""
...
#os.environ['TORCH_HOME'] = os.path.dirname(__file__)+'../../pretrained_models'
import timm
...
class ASTModel(nn.Module):
    ...
    def __init__(self, label_dim=527, fstride=10, tstride=10, input_fdim=128, input_tdim=1024, imagenet_pretrain=True, audioset_pretrain=False, model_size='base384', verbose=True,
        ...
        # now load a model that is pretrained on both ImageNet and AudioSet
            ...
            self.v.pos_embed = nn.Parameter(torch.cat([self.v.pos_embed[:, :2, :].detach(), new_pos_embed], dim=1))
            self.v.blocks=self.v.blocks[:number_of_layers]
